...
export class LoadShowOrPage extends SingletonAction<JsonObject> {
	...
	override async onWillAppear(ev: WillAppearEvent<JsonObject>): Promise<void> {
		const svgPage = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><!--!Font
                        Awesome Free 6.7.2 by @fontawesome - https://fontawesome.com License -
                        https://fontawesome.com/license/free Copyright 2025 Fonticons, Inc.-->
                        <g transform="scale(0.65) translate(200, 137.8)">
                        <path fill="#ffffff"
		...
		if (page !== undefined) {
			...
			await ev.action.setImage(`data:image/svg+xml,${encodeURIComponent(svgPage)}`);
		} else {
