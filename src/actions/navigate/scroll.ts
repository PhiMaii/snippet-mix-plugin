...
export class Scroll extends SingletonAction {
	...
	override async onKeyDown(ev: KeyDownEvent<ScrollSettings>) {
		...
		RenderScroll(ev.action, ev.payload.settings);
	}
...
export async function RenderScroll(action: any, settings?: ScrollSettings) {
	streamDeck.logger.info("Current Scorll Offset: ", SCROLL_OFFSET);
	streamDeck.logger.info("Current Scorll Offset: ", settings);
